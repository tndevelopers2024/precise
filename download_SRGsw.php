...
function render_email($email, $data) {
    ob_start();
    include "email_temp_SRGsw.phtml";
    return ob_get_contents();
}

if($_POST)
{
	$data['namesrg'] = isset($_POST['name_srg']) ? $_POST['name_srg']:'';
	$data['emailsrg'] = isset($_POST['email_srg']) ? $_POST['email_srg']:'';
	$data['contactnumbersrg'] = isset($_POST['contactnumber_srg']) ? $_POST['contactnumber_srg']:'';
	$body = render_email('email', $data);

	$subject = "You have a message from your client for system requirements guide";
