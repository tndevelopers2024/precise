...
function render_email($email, $data) {
    ob_start();
    include "email_temp_DIGsw.phtml";
    return ob_get_contents();
}

if($_POST)
{
	$data['namedig'] = isset($_POST['name_dig']) ? $_POST['name_dig']:'';
	$data['emaildig'] = isset($_POST['email_dig']) ? $_POST['email_dig']:'';
	$data['contactnumberdig'] = isset($_POST['contactnumber_dig']) ? $_POST['contactnumber_dig']:'';
	$body = render_email('email', $data);

	$subject = "You have a message from your client for downloading installation guide";
	...
	$mail->From = $from;
	$mail->FromName = "Customer Submitted a form from Geomagic Solid works";
...
try {
 ...
 header("Location: https://www.precise3dm.com/geomagic-for-solidworks-reverse-engineering-software.php#geoform");
