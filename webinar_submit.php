...
function render_email($email, $data) {
    ob_start();
    include "email_temp_webinar.phtml";
    return ob_get_contents();
}

if($_POST)
{
	$data['cadName'] = isset($_POST['cadName']) ? $_POST['cadName']:'';
	$data['cadEmail'] = isset($_POST['cadEmail']) ? $_POST['cadEmail']:'';
	$data['cadContnum'] = isset($_POST['cadContnum']) ? $_POST['cadContnum']:'';
	$data['cadLocation'] = isset($_POST['cadLocation']) ? $_POST['cadLocation']:'';
	$data['cadCompanyname'] = isset($_POST['cadCompanyname']) ? $_POST['cadCompanyname']:'';
	$body = render_email('email', $data);

	$subject = "You have a message from your client for cad";
	...
	$mail->From = $from;
	$mail->FromName = "Get Quote request from cad";
