...
function render_email($email, $data) {
    ob_start();
    include "email_temp_DSBsw.phtml";
    return ob_get_contents();
}

if($_POST)
{
	$data['namedsb'] = isset($_POST['name_dsb']) ? $_POST['name_dsb']:'';
	$data['emaildsb'] = isset($_POST['email_dsb']) ? $_POST['email_dsb']:'';
	$data['contactnumberdsb'] = isset($_POST['contactnumber_dsb']) ? $_POST['contactnumber_dsb']:'';
	$body = render_email('email', $data);

	$subject = "You have a message from your client for download software brochure";
